...
public class ExternalMergeCheckHook implements RepositoryMergeCheck, RepositorySettingsValidator {
  private static Logger log = Logger.getLogger(ExternalMergeCheckHook.class.getSimpleName());
  ...
  private static String cloneUrlFromRepository(
      REPO_PROTOCOL protocol, Repository repository, RepositoryService repoService) {
  ...
  public RepositoryHookResult preUpdate(
    ...
    // compat with < 3.2.0
    String repoPath =
        this.properties.getRepositoryDir(pr.getFromRef().getRepository()).getAbsolutePath();
    ...
    refChanges.add(new ExternalRefChange(
        pr.getToRef().getId(),
    ...
    env.put(
        ...
        pr.getFromRef().getRepository().getProject().getId() + "");
    env.put(
        "PULL_REQUEST_FROM_REPO_PROJECT_KEY",
        pr.getFromRef().getRepository().getProject().getKey());
    ...
    env.put(
        "PULL_REQUEST_TO_REPO_PROJECT_ID", pr.getToRef().getRepository().getProject().getId() + "");
    env.put(
        "PULL_REQUEST_TO_REPO_PROJECT_KEY", pr.getToRef().getRepository().getProject().getKey());
    ...
    // env.put("PULL_REQUEST_REVIEWERS", iterableToString(transform(pr.getReviewers(), (p) ->
    // p.getUser().getDisplayName())));
    // env.put("PULL_REQUEST_REVIEWERS_ID", iterableToString(transform(pr.getReviewers(), (p) ->
    // Integer.toString(p.getUser().getId()))));
    // env.put("PULL_REQUEST_REVIEWERS_SLUG", iterableToString(transform(pr.getReviewers(), (p) ->
    // p.getUser().getSlug())));
    // env.put("PULL_REQUEST_REVIEWERS_APPROVED_COUNT",
    // Integer.toString(newArrayList(filter(pr.getReviewers(), isApproved)).size()));
    // env.put("PULL_REQUEST_PARTICIPANTS_APPROVED_COUNT",
    // Integer.toString(newArrayList(filter(pr.getParticipants(), isApproved)).size()));
    ...
    final String summaryMsg =
        wasAccepted ? "External hooks check was successful." : "External hooks check failed.";
    ...
    this.comment = StringUtils.isEmpty(detailMsg)
        ? summaryMsg
        : String.format("%s\n%s", summaryMsg, detailMsg);
  ...
  public void onEnd(
    ...
    if (BooleanUtils.isTrue(settings.getBoolean("add_comments"))) {
      ...
      this.commentService.addComment(new AddCommentRequest.Builder(pr, this.comment).build());
    }

    if (result.isRejected()
        && BooleanUtils.isTrue(settings.getBoolean("decline_pull_request_on_rejection"))) {
      log.log(FINER, "Declining Pull Request as configured");
      this.pullRequestService.decline(
          new PullRequestDeclineRequest.Builder(pr, pr.getVersion()).build());
    }
  ...
  private Optional<PullRequestCheck> getCheck(PullRequest pr) {
    ...
    final PullRequestCheck[] checks = this.activeObjects.find(PullRequestCheck.class, Query.select()
        .where(
  ...
  // private static final Predicate<PullRequestParticipant> isApproved = new
